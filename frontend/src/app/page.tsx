...
const mockNodes = [
  { id: 'n1', name: 'Disk 1 (Node A)', max: 104857600, used: 15728640 },
  { id: 'n2', name: 'Disk 2 (Node B)', max: 104857600, used: 8388608 },
  { id: 'n3', name: 'Disk 3 (Node C)', max: 104857600, used: 0 },
];
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNodes((prev) => {
          const newNodes = [...prev];
          newNodes[0].used += file.size; // Just add to disk 1 for visual feedback
          return newNodes;
        });
      } else {
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-8 font-sans">
      ...
      <main className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-2 bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
          <h2 className="text-xl font-semibold mb-6">Storage Nodes (Metadata)</h2>
          
          <div className="space-y-6">
            {nodes.map(node => {
              const percentage = Math.min(100, (node.used / node.max) * 100);
              return (
                <div key={node.id} className="bg-neutral-950 rounded-xl p-5 border border-neutral-800">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-xs text-neutral-500 font-mono">
                      {(node.used / 1024 / 1024).toFixed(2)} MB / {(node.max / 1024 / 1024).toFixed(0)} MB
                    </span>
                  </div>
